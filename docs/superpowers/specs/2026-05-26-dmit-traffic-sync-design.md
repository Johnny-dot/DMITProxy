...
## 真机验证发现（2026-06-03，必读）

单元/集成测试全绿 ≠ 真的能用。以下是只有真机才暴露、且影响设计的事实：

1. **Cloudflare 按频率挑战。** `get_rules` 接口在 DMIT 的 Cloudflare 后面。单次（带有效 `cf_clearance`、普通浏览模式）能拿到 200 JSON，但**短时间内连发多次会触发 "Just a moment" 挑战页（HTTP 403 + HTML）**，且这个挑战会**连带挡住 DMIT 自己 app 的请求**一小段时间。
   - ⇒ 油猴脚本**必须节流**：`GM_getValue/GM_setValue` 记录上次同步时间，最少间隔 30 分钟，且时间戳在请求**之前**写入（失败也计入,绝不猛刷）。月度配额仪表盘变化很慢,低频完全够。
2. **DevTools 设备模拟模式会误触发挑战。** 在"响应式/设备模拟"下,`Save-Data` 头 + 模拟 UA 会让 Cloudflare 直接 403。普通窗口正常。（调试时别在模拟模式下测这个接口。）
3. **`auto_min_days_until_due` 不是常驻字段。** 高用量(如 69.58%)时返回 `"8.57 天"`;**流量刚重置、低用量(0.42%)时该条件不返回**,`days_until_reset_text` 为 null。
   - ⇒ billing-day 自动检测**只能当"有就用"的机会主义功能**,不能依赖。后端已对缺失值 noop,不崩。运营上:**在 admin 面板手动设一次 billing day(本机为每月 3 日)** 才是可靠做法。
4. **核心目标已真机验证。** 同一时刻 fetch 拿到的 `bwusage/bwlimit` 换算后与 DMIT 仪表盘**逐字段吻合**(已用 4.16 GB、剩余 995.84 GB),证明 `机器余量` 同步口径正确 —— 这是本特性的主要价值,已确认成立。
