...
export function MarketTab() {
  ...
  const openNewsLink = useCallback((url: string) => {
  ...
  return (
    <div className="space-y-4" data-testid="portal-market-tab">
      <section className="grid gap-4 xl:grid-cols-[minmax(0,1.45fr)_minmax(340px,0.95fr)] xl:items-start">
        <div className="min-w-0 space-y-4">
                    ...
                    <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                      ...
                      <div className="space-y-2 md:text-right">
                        ...
                        >
                          ...
                          <span>
                            {formatMarketDelta(selectedItem.change, selectedItem.decimals)}
                          </span>
          ...
          <div className="hidden xl:block">{quickScanSection}</div>
        </div>
        ...
        {isLoadingNews ? (
          ...
          <aside className="surface-card space-y-5 p-5 md:p-6 xl:sticky xl:top-24">
            <div className="flex items-start justify-between gap-4">
              <div className="space-y-2">
                <p className="section-kicker">{isZh ? '资讯' : 'News'}</p>
                <h2 className="text-xl font-semibold tracking-tight text-zinc-50">
                  {isZh ? '最新资讯与 AI 访谈。' : 'Latest headlines and AI talks.'}
                </h2>
                <p className="text-sm leading-6 text-zinc-400">
                  {isZh
                    ? '按主题浏览头条，点开即可回到原站阅读全文。'
                    : 'Browse headlines, interviews, and podcast links by topic, then open the source site whenever you want the full story.'}
                </p>
              </div>
              <Button
                variant="outline"
            ...
            {newsFeed ? (
              <>
                <div className="rounded-[28px] border border-[var(--border-subtle)] bg-[var(--surface-panel)] p-3">
                  <div className="flex flex-wrap items-center gap-2 text-xs text-[var(--text-secondary)]">
                    <span className="rounded-full border border-[var(--border-subtle)] bg-[var(--surface-card)] px-3 py-2">{`${isZh ? '更新' : 'Updated'} ${formatDateTime(newsFeed.cachedAt, locale)}`}</span>
                    <a
                      href={newsFeed.attributionUrl}
                      target="_blank"
                      rel="noreferrer"
                      className="inline-flex items-center gap-1 text-[var(--success)] hover:text-[var(--accent-strong)]"
                    >
                      {newsFeed.provider}
                      ...
                      <button
                        ...
                        className={cn(
                          ...
                          topic.id === selectedTopicId
                            ...
                            : 'border-[color:var(--border-subtle)] bg-[var(--surface-card)] text-[var(--text-secondary)] hover:text-zinc-100',
                        )}
                        onClick={() => {
                ...
                {selectedTopic && selectedHeadline ? (
                  <>
                    <div className="rounded-[30px] border border-[var(--border-strong)] bg-[var(--surface-strong)] p-4 shadow-[0_20px_50px_rgba(0,0,0,0.18)] md:p-5">
                      <div className="flex flex-wrap items-center justify-between gap-3">
                        <span className="inline-flex items-center rounded-full border border-[var(--success-soft-strong)] bg-[var(--success-soft)] px-3 py-1 text-[11px] font-medium uppercase tracking-[0.18em] text-[var(--success)]">
                          {selectedTopic.id === 'aiTalks'
                            ? isZh
                              ? '焦点访谈'
                              : 'Featured talk'
                            : isZh
                        ...
                        <span className="text-xs text-[var(--text-secondary)]">
                          {isZh
                            ? `${selectedTopic.items.length} 条同主题内容`
                            : `${selectedTopic.items.length} items in this topic`}
                        </span>
                      </div>
                      ...
                      <h3 className="mt-3 text-xl font-semibold leading-8 text-zinc-50">
                      ...
                      <div className="mt-5 flex flex-wrap items-center justify-between gap-3">
                        <div className="flex flex-wrap items-center gap-3 text-xs text-[var(--text-secondary)]">
                          ...
                          {selectedHeadline.sourceUrl ? (
                            <a
                              ...
                              target="_blank"
                              rel="noreferrer"
                              className="text-[var(--success)] hover:text-[var(--accent-strong)]"
                            >
                        ...
                        <Button
                          size="sm"
                          className="gap-2"
                          onClick={() => openNewsLink(selectedHeadline.url)}
                        >
                      ...
                      <div className="mb-3 flex items-end justify-between gap-3 px-1">
                        <div>
                          <p className="text-[11px] uppercase tracking-[0.16em] text-[var(--text-secondary)]">
                            {isZh ? '更多头条' : 'More in this topic'}
                          </p>
                          <p className="mt-1 text-sm text-[var(--text-secondary)]">
                            {isZh
                              ? `${isZh ? selectedTopic.labelZh : selectedTopic.labelEn} 共 ${selectedTopic.items.length} 条`
                              : `${selectedTopic.items.length} headlines in ${isZh ? selectedTopic.labelZh : selectedTopic.labelEn}`}
                          </p>
                        </div>
                        <span className="rounded-full border border-[var(--border-subtle)] bg-[var(--surface-card)] px-3 py-1 text-xs text-[var(--text-secondary)]">
                      ...
                      <div className="space-y-2">
                        {selectedTopic.items.map((item, index) => (
                          <div
                            key={item.id}
                            className={cn(
                              'group rounded-[20px] border px-3 py-3 transition-colors',
                          ...
                          >
                            <div className="flex items-start gap-3">
                              ...
                              >
                                <div
                                  className={cn(
                                    ...
                                    item.id === selectedHeadline.id
                                      ? 'border-[var(--success-soft-strong)] bg-[var(--success-soft)] text-[var(--success)]'
                              ...
                              <a
                                ...
                                aria-label={
                                  isZh
                                    ? ['打开原文：', item.title].join('')
                                    : `Open original: ${item.title}`
                                }
                                className="mt-0.5 inline-flex h-8 w-8 shrink-0 items-center justify-center rounded-full border border-[var(--border-subtle)] bg-[var(--surface-panel)] text-[var(--text-secondary)] transition-colors hover:border-[var(--success-soft-strong)] hover:bg-[var(--success-soft)] hover:text-[var(--success)]"
                              >
                                <ExternalLink className="h-4 w-4" />
                              </a>
                            </div>
      ...
      <div className="xl:hidden">{quickScanSection}</div>
    </div>
  );
}
