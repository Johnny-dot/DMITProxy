...
export function NotificationsPanel({
  ...
  // 180ms matches the .anim-side-panel-right and .anim-backdrop exit animations.
  const { mounted, animState } = useDelayedUnmount(open, 180);
  ...
  return createPortal(
    <>
      <div
        ...
        onClick={onClose}
        className="anim-backdrop fixed inset-0 z-[80] bg-[color:var(--overlay)] backdrop-blur-sm"
      />
      <aside
        data-anim-state={animState}
        className="anim-side-panel-right fixed inset-x-4 bottom-4 top-4 z-[90] flex flex-col sm:left-auto sm:w-[410px]"
      >
          ...
          <div className="soft-divider border-b px-5 py-5">
            ...
            {viewerName && (
              <div className="glass-pill mt-4 inline-flex items-center gap-2 px-3 py-2">
                ...
                <span className="text-sm font-medium text-[var(--text-primary)]">{viewerName}</span>
              </div>
            )}
            {adminMode && (
              <div className="surface-panel mt-4 p-1">
                <div className="grid grid-cols-2 gap-1">
                  <button
                    ...
                    className={cn(
                      'px-3 py-2 text-left',
                      adminView === 'announcements' ? 'glass-nav-item-active' : 'glass-nav-item',
                    )}
                  >
                    <div className="flex items-center justify-between gap-2">
                  ...
                  >
                    <div className="flex items-center justify-between gap-2">
                      <span className="text-sm font-medium">{isZh ? '系统提醒' : 'System'}</span>
                      <Badge
              ...
              adminView === 'announcements' ? (
                ...
                <section className="space-y-3">
                  <div className="flex items-start justify-between gap-3 px-1">
                    ...
                    <Badge variant={unreadCount > 0 ? 'success' : 'secondary'}>
                      {isZh ? `${notifications.length} 条提醒` : `${notifications.length} items`}
                    </Badge>
                  </div>
