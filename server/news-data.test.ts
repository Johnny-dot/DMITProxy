...
describe('news data aggregation', () => {
  it('merges multiple sources, extracts covers and summaries, and serves from cache', async () => {
    ...
    try {
      ...
      expect(markets?.items[0]).toMatchObject({
        source: 'MarketWatch',
        imageUrl: 'https://cdn.example.com/marketwatch.jpg',
      });
      expect(
        technology?.items.some(
          (item) => item.imageUrl === 'https://cdn.example.com/verge-cover.jpg',
        ),
      ).toBe(true);
      expect(technology?.items.some((item) => item.summary?.includes('local AI features'))).toBe(
        true,
      );
      ...
      expect(crypto?.items[0]).toMatchObject({
        source: 'CoinDesk',
        imageUrl: 'https://cdn.example.com/coindesk-cover.jpg',
      });
