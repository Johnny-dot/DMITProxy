...
$artifactDir = Join-Path $repoRoot "output\playwright\$runStamp"
$dataDir = Join-Path $repoRoot ".tmp\layout-audit\$runStamp\data"
$serverPort = 3301
$vitePort = 3300
$baseUrl = "http://127.0.0.1:$vitePort"
$serverOutLog = Join-Path $artifactDir "server.out.log"
$serverErrLog = Join-Path $artifactDir "server.err.log"
$viteOutLog = Join-Path $artifactDir "vite.out.log"
$viteErrLog = Join-Path $artifactDir "vite.err.log"
...
"@
  ...
  $scriptArgs = @(
    "tsx",
    "scripts/playwright/layout-audit.ts",
    "--base-url",
    $baseUrl,
    "--data-dir",
    $dataDir,
  ...
  if ($LASTEXITCODE -ne 0) {
    throw "Layout audit failed with exit code $LASTEXITCODE"
  }
} finally {
