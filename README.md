...
| `PUBLIC_NODE_HOST`                          | No              | Public hostname written into generated proxy nodes. Point this DNS-only record at the Xray node, not the 3X-UI admin panel. / 写入订阅节点地址栏的公开域名；应使用 DNS-only 记录指向 Xray 节点，而不是 3X-UI 管理面板。                           |
